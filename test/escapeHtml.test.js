...
describe('escapeHtml', () => {
  ...
  it('keeps leading and trailing test', () => {
    expect(escapeHtml('leading & trailing')).toBe('leading &amp; trailing');
  });

  it('escapes multiple segments', () => {
    expect(escapeHtml('> to >')).toBe('&gt; to &gt;');
  });

  it('escapes complex string', () => {
    expect(
      escapeHtml(
        '<div onClick="javascript:alert(\'inject\')">Hello, world! This, that & those.</div>'
      )
    ).toBe(
      '&lt;div onClick=&quot;javascript:alert(&#39;inject&#39;)&quot;&gt;Hello, world! This, that &amp; those.&lt;/div&gt;'
    );
